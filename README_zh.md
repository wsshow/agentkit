...
文件存储使用安全的哈希文件名，并对临时文件、原子替换后的目录元数据进行同步，避免非法会话 ID 造成路径穿越，也避免进程异常时留下半个 JSON 或未提交的快照。管理会话可直接使用存储：
...
请求/响应服务无需自己编写 goroutine 管理代码，即可在后台启动持久化目标：

```go
run, err := goals.StartAsync(workerCtx, agentkit.GoalRequest{
    Objective: "准备并验证 v2 版本发布",
})
goalID := run.ID() // 已经落盘，现在可安全返回给客户端
...
旧式 SSE 服务器可使用 `MCPTransportSSE`。每个服务器的连接与首次完整分页工具发现共用默认 30 秒时限，可通过 `InitializationTimeout` 调整；调用方 context 更早的截止时间仍然优先。后续初始化失败时的资源清理也使用相同的有界时长；若自定义 Session 忽略 `Close`，清理会留在后台继续而不会永久卡住 `New`。`New` 只读取一次工具列表，服务器之后增删工具时需重建 Agent。暴露给模型的工具名必须唯一，因此当多个服务器或本地工具重名时应设置 `ToolPrefix`。`ToolNames` 中服务器未提供的名字会直接导致初始化失败，不会静默遗漏。
