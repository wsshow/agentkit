...
Every successful `Load` must return a non-nil object whose ID exactly matches the requested ID. Session, goal, and tool-result IDs are exact keys and cannot contain surrounding whitespace; owning session IDs follow the same rule. Goal snapshots must also contain a valid status, session ID, objective, and positive iteration limit. `List` results must carry valid IDs, lifecycle values, counts or sizes, and timestamps without duplicate entries. AgentKit validates these boundaries and returns `ErrInvalidPersistenceData` for malformed backend output instead of risking a panic, cross-record restore, or unsafe retention delete. Loaded and listed data is copied before AgentKit uses it, including checkpoint byte slices in both directions, so a backend may safely retain its own cache objects.
...
After that conflict, the losing Agent is marked stale and rejects further model or tool execution with `ErrSessionStale`. Recreate a directly managed Agent with the same ID to load the authoritative snapshot. When using `SessionManager`, simply call `Open`: it closes the stale instance and returns a freshly restored Agent. Divergent in-memory messages are deliberately not merged or retried.
