...
The default model-based evaluator reuses the Agent's `ModelRetryConfig`, including custom retry decisions and backoff. Transient evaluation failures therefore receive the same protection as normal Agent model calls without another configuration block. A custom `GoalEvaluator` remains fully under application control.
