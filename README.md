...
The default model-based evaluator reuses the Agent's `ModelRetryConfig` and `ModelFailoverConfig`, including custom retry decisions, backoff, and alternate model selection. It exhausts retries on the current model before failing over, matching normal Agent calls. Transient evaluation failures therefore receive the same protection without another configuration block. A panic from a custom `GoalEvaluator` is returned as an error wrapping `ErrGoalEvaluatorPanic`; the already completed work remains committed with a pending evaluation and can be resumed without repeating it.
...
agent, err := agentkit.New(ctx, &agentkit.Config{
    ...
    MCP: &agentkit.MCPConfig{
        InitializationTimeout: 30 * time.Second, // optional; this is the default per server
        Servers: []agentkit.MCPServerConfig{
            {
                Name:       "search",
                Transport:  agentkit.MCPTransportStreamableHTTP,
...
`MCPTransportSSE` is available for legacy SSE servers. Each server's connection and initial fully paginated tool discovery share a 30-second default deadline, configurable with `InitializationTimeout`; the caller's earlier context deadline still wins. Cleanup after a later initialization failure uses the same bounded duration and continues in the background if a custom session ignores `Close`. Tool lists are loaded once during `New`, so recreate the agent to pick up later additions or removals. Exposed tool names must be unique, so configure `ToolPrefix` when servers—or local tools—use the same name. A requested `ToolNames` entry that the server does not provide is an initialization error instead of a silent omission.
